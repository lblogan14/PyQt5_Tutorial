...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		# SIGNAL: The connected function will be called whenever
		# the window title is changed. The new title is discarded 
		# in the lambda and the function is called without parameters.
		self.windowTitleChanged.connect(lambda x: self.my_custom_fn())

		# SIGNAL: The connected function will be called whenever
		# the window title is changed. The new title is passed to
		# the function and replaces the default parameter. Extra data
		# is passed from within the lambda.
		self.windowTitleChanged.connect(lambda x: self.my_custom_fn(x, 25))

		# This sets the window title which will trigger all the above signals
		# sending the new title to the attached fucntions or lambdas
		# as the first parameter.
		self.setWindowTitle('This will trigger all the signals.')

	# SLOT: This accepts a string, e.g., the winow title, and prints it
